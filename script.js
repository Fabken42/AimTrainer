...
function adicionaAlvo() {
    ...
    let y = Math.floor(Math.random() * (canvas.height - (RAIO_MAXIMO * 4) - (y_texto * 2)) + (RAIO_MAXIMO * 2) + y_texto);
    alvos.push(new Alvo(x, y, 1));
}

function tocaSom(nome) {
    if (nome === "perde-vida") {
        var audio = new Audio('./audio/failureSound.wav');
        audio.volume = 0.5;
        audio.play();
    } else if (nome === "acerto") {
        var audio = new Audio('./audio/hitSound.wav');
        audio.volume = 0.5;
        audio.play();
    } else {
        var audio = new Audio('./audio/clappingAudio.wav');
        audio.volume = 0.5;
        audio.play();
    }
}
